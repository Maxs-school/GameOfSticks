...
namespace GameOfSticksTest
{
	...
	public class UnitTest
	{
		...
		public UnitTest()
		{
			...
			this.ai = new AI(MaxSticksInGame,MaxSticksPerTurn);
		...
		public void CheckOneLeft()
		{
			...
			{
				Assert.AreEqual(1, this.ai.MakeMove(1));
				this.ai.TrainStep(false);
			}
		}
		...
		private int[] PlayGame()
		{
			...
			{
				// AI move
				int taken = this.ai.MakeMove(sticksRemaining);
				...
				for (int slot = 0; slot < MaxSticksPerTurn; slot++)
				{
					if (cup[slot] == startingBrain[i][slot])
					{
						if (slot == taken - 1)
						{
							...
							Assert.IsTrue(!victoryToAI && cup[slot] == 1, // The original message was not desciptive
								"Slot content stayed the same at 1 but this slot was was used and the ai won");
						}
					}
					else
					{
